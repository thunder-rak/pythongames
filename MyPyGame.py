...
WIDTH, HEIGHT = 600, 844 #DIMENSIONS OF SCREEN (x,y)
...
def main():
    ...
    while run:
        ...
        if b1.y + 50 > HEIGHT:
            bullet = pygame.Rect(WIDTH/2-b1.width/2,0, b1.width, b1.width)
            if len(b1_bullets) <= MAX:
             b1_bullets.append(bullet)
            b1.y = HEIGHT - 49
        else:
            b1.y += 1
        b1.y +=1
        if keys_pressed[pygame.K_UP]and b2.y - VELOCITY>0:#up key for WASD
            b2.y -= VELOCITY
        if keys_pressed[pygame.K_DOWN]and b2.y + b2.height<HEIGHT:#down key for WASD
            b2.y += VELOCITY
        if len(b1_bullets) > 0:
         b1_bullets[-1 + MAX].y += 1
         if b1_bullets[-1 + MAX].y > HEIGHT - 50:
             b1_bullets[-1 + MAX].y = HEIGHT - 49
             MAX +=1
